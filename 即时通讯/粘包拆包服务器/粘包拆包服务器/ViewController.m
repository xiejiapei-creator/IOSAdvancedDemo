...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if (self.dataM.length == self.totalSize)
    {
        ...
        else if  (self.currentCommandId == kcVideoDataType)// 视频
        {
            [self saveVideo];
        }
        else if  (self.currentCommandId == kcTextDataType)// 文本
        {
            NSData *textData = [self.dataM subdataWithRange:NSMakeRange(8, self.dataM.length - 8)];
            NSString *text = [[NSString alloc] initWithData:textData encoding:kCFStringEncodingUTF8];
            NSLog(@"服务器接收到客户端发来的文本：%@",text);
...
- (void)saveVideo
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        NSLog(@"写入视频状态: %d 路径: %@",success,videoPath);
